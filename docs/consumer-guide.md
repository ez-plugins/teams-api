---
title: Consumer Guide
nav_order: 3
has_children: true
description: "Start here if you are building an addon plugin that consumes team data"
---

# Consumer Guide

This section is for addon plugin developers who want to read or react to team data.

Recommended reading order:

1. [Consumer Tutorial (Bukkit)](consumer-tutorial-bukkit)
2. [Registering Subcommands](consumer-subcommands)
3. [Consumer Tutorial (Proxy)](consumer-tutorial-proxy)
4. [Velocity Guide](velocity) or [BungeeCord Guide](bungeecord)
5. [API Reference](api)
