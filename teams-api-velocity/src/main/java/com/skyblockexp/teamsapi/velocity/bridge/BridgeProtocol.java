 ...
 *
 * <p><b>Request format</b> (Velocity → backend):</p>
 * <pre>{@code {"reqId":"<uuid>","op":"<operation>","<argKey>":"<argVal>",...}}</pre>
 *
 * <p><b>Response format</b> (backend → Velocity):</p>
...
public final class BridgeProtocol {
     ...
     */
    public static java.util.Map<String, String> parseResponse(final byte[] data) {
        ...
        // Extract the raw JSON body between the outer braces for field parsing
        final int start = json.indexOf('{');
        final int end = json.lastIndexOf('}');
        if (start < 0 || end <= start) {
            return result;
        }
        // Simple tokeniser: walk pairs of "key":value separated by commas
        ...
        while (!body.isEmpty()) {
            ...
            if (!body.isEmpty() && body.charAt(0) == '"') {
                ...
                // Unquoted value (bool, number, null, or nested structure)
                int vEnd = 0;
                int depth = 0;
     ...
     * Returns {@code null} if the key is absent or the value is the JSON literal
     * {@code null}.
     *
     ...
     */
    public static String getString(final java.util.Map<String, String> map, final String key) {
     ...
     */
    public static boolean getBool(final java.util.Map<String, String> map,
     ...
     */
    public static int getInt(final java.util.Map<String, String> map,
